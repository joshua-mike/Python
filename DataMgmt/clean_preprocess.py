...
print(f"Data after filling missing values: {df.isnull().sum()}")
df = remove_outliers_zscore(df)
print(f"Data after removing outliers: {df.describe()}")
# df = cap_outliers(df)
# df = min_max_scaling(df)
df = z_score_standardization(df)
print(f"Data after standardization: {df.head()}")
df = one_hot_encoding(df)
print(f"Data after one-hot encoding: {df.columns}")
